...
void udpBind(uint16_t port)
{
	...
	status = qcom_bind(udpSock, (struct sockaddr *)(&localAddr), sizeof(localAddr)) ? A_OK : A_ERROR;
...
void httpGet(const char *reqdata, char *respdata)
{
	...
	char hostname[128]={0};
	for(int i=0; ((reqdata[i]!='/') && (reqdata[i]!=0) && i<128); i++)
	...
	do
	{
		...
		status = qcom_connect(sock, (struct sockaddr *)&addr, sizeof(addr));
